...
/// <summary>
/// Handles ALSA mixer operations for volume and mute control.
/// </summary>
internal class AlsaMixerDevice : IDisposable
{
    ...
    {
        SetPlaybackVolume(NativeWidth.ToNint(volume));
    }
    ...
    {
        SetRecordingVolume(NativeWidth.ToNint(volume));
    }
    ...
    private void SetPlaybackVolume(nint volume)
    {
        OpenMixer();
        try
        {
    ...
    private void SetRecordingVolume(nint volume)
    {
        OpenMixer();
        try
        {
    ...
    private IntPtr FindMixerElement(string targetName)
    {
        ...
        while (element != IntPtr.Zero)
        {
            ...
            {
                _log?.Error(ex, "[ALSA] Error reading mixer element name");
            }
        ...
        snd_mixer_selem_channel_id channel, nint value)
    {
        try
        {
            ...
            if (IsAllChannels(channelName))
            {
                int result = InteropAlsa.snd_mixer_selem_set_playback_switch_all(element, switchValue);
                if (result >= 0) return result;

                result = InteropAlsa.snd_mixer_selem_set_capture_switch_all(element, switchValue);
                if (result >= 0) return result;
            }
            else
            {
                int result = InteropAlsa.snd_mixer_selem_set_playback_switch(element, channel, switchValue);
                if (result >= 0) return result;

                result = InteropAlsa.snd_mixer_selem_set_capture_switch(element, channel, switchValue);
                if (result >= 0) return result;
            }
        }
        catch (Exception ex)
        {
            _log?.Error(ex, "[ALSA] Failed to set switch value");
            throw;
        }
    ...
    private int TrySetFallbackElement(nint value)
    {
        ...
        if (InteropAlsa.snd_mixer_selem_has_playback_volume(_mixerElement) != 0)
        {
            return InteropAlsa.snd_mixer_selem_set_playback_volume(_mixerElement, 
                snd_mixer_selem_channel_id.SND_MIXER_SCHN_FRONT_LEFT, value);
        }
    ...
    private void OpenMixer()
    {
        ...
        lock (MixerLock)
        {
            ...
            IntPtr mixer;
            AlsaErrorHandler.ValidateResult(
                InteropAlsa.snd_mixer_open(out mixer, 0), 
                ExceptionMessages.CanNotOpenMixer, _log);
            _mixer = mixer;

            AlsaErrorHandler.ValidateResult(
                InteropAlsa.snd_mixer_attach(_mixer, _settings.MixerDeviceName), 
                ExceptionMessages.CanNotAttachMixer, _log);
            
            AlsaErrorHandler.ValidateResult(
                InteropAlsa.snd_mixer_selem_register(_mixer, IntPtr.Zero, IntPtr.Zero), 
                ExceptionMessages.CanNotRegisterMixer, _log);
            
            AlsaErrorHandler.ValidateResult(
                InteropAlsa.snd_mixer_load(_mixer), 
                ExceptionMessages.CanNotLoadMixer, _log);
    ...
    private void CloseMixer()
    {
        ...
        lock (MixerLock)
        {
            ...
            AlsaErrorHandler.ValidateResult(
                InteropAlsa.snd_mixer_close(_mixer), 
                ExceptionMessages.CanNotCloseMixer, _log);

            _mixer = IntPtr.Zero;
            _mixerElement = IntPtr.Zero;
        }
    }
